...
export default function Home() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // redirecionar ou guardar token aqui
      } else {
  ...
  return (
    <div className="h-screen w-full bg-slate-500 flex items-center justify-center">
      <form onSubmit={handleSubmit} className="flex flex-col h-96 w-96 bg-white rounded-lg p-6 shadow-lg">
        ...
        <p className="text-slate-400 mt-5 flex justify-center text-sm">
          Not a member? 
          <a className="underline px-1 cursor-pointer hover:text-gray-800" href="/signup">Sign up now</a>
        </p>
      </form>
    </div>
  );
